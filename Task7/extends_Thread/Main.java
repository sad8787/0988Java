...
class ProxyCheker extends Thread{
    String ip;
    int port;
    ProxyCheker(String ip, int port){
        this.ip=ip;
        this.port=port;
    }
    @Override
    public void run() {
        try{
            ...
            rd.close();
            FileOutputStream fos = new FileOutputStream("C://java/good_ip.txt", true);//aqui true para que no borre lo que ya esta
            byte[] buffer = (ip+":"+port+"\n").getBytes();
            fos.write(buffer);
            fos.close();
        }catch (IOException e){
